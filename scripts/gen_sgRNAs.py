...
Usage:
    gen_sgRNAs.py [-ch] <gens_file> <targ_file> <locus> <pams_dir> <ref_fasta> <out_dir> <cas_types> <guide_length> [<gene_vars>] [--crispor] [<ref_gen>] [--hom] [--multi] [--max_indel=<S>]
...
def parse_args(args, spec_locus=False):
    ...
    if spec_locus:
        ...
        locus = args['<locus>']
    return args['<out_dir>'], args['<pams_dir>'], args['<gens_file>'], args['<targ_file>'], locus, ref, CAS_LIST, guide_length, int(args['--max_indel']) 
...
def verify_hdf_files(gen_file, targ_file, chrom, start, stop, max_indel):
    """
    ...
    return gen_file[indel_too_large], targ_file[indel_too_large]
...
def get_allele_spec_guides(args, spec_locus=False):
    ...
    out_dir, pams_dir, gens, targ_gens, locus, ref_genome, CAS_LIST, guide_length, max_indel = parse_args(args, spec_locus)
...
>>>>>>> 56106339d88ac99b84d70f3d46cce4040e15e59d
    grna_df = grna_df.query('variant_position_in_guide != 2')


    # add specificity scores if specified
    if args['--crispor']:
        out = get_crispor_scores(grna_df, out_dir, args['<ref_gen>'])
...
def main(args):
    ...
    out.to_csv(args['<out_dir>'] + '_guides.tsv', sep='\t', index=False)
    print('Done.')
