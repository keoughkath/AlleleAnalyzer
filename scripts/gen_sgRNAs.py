...
Usage:
    gen_sgRNAs.py <gens_file> <locus> <pams_dir> <ref_fasta> <out_dir> <cas_types> <guide_length>
...
def makes_pam(chrom, pos, ref, alt, ref_genome):
    ...
    for cas in CAS_LIST:
        ...
        if len(alt_pams_for) - len(ref_pams_for) > 0 or len(alt_pams_rev) - len(ref_pams_rev) > 0:
            makes_pam[counter] = 1
    alt_pams_for = map(lambda x:x + pos - 10, alt_pams_for)
    alt_pams_rev = map(lambda x:x + pos - 10, alt_pams_rev)
    return bool(makes_pam), alt_pams_for, alt_pams_rev


def get_made_broke_pams(df, chrom, ref_genome, cas_list, guide_length, pams_dir):
    # df is gens
    ...
    for index, row in df.iterrows():
        ...
        for cas in cas_list:
            if cas in crisprtools.tpp_for.keys():
                ...
                alt_pams_for = list(filter(lambda x:x not in pam_for_pos, alt_pams_for))
                alt_pams_rev = list(filter(lambda x:x not in pam_rev_pos, alt_pams_rev))
                if made_pam:
    ...
    return starts, stops, refs, alts, grna_refs, grna_alts, variant_pos_in_guides, cas_types, chroms, variants_positions, strands, pam_pos
...
def main(args):
        ...
        if cas in TP_CAS_LIST:
            ...
            for pos in pam_rev_pos:
                ...
                for index, row in prox_var_df.iterrows():
                    ...
                    start = pos
                    starts.append(start)
                    stops.append(pos + guide_length + 1)
                    ref_allele = row['ref']
                    refs.append(ref_allele)
                    alt_allele = row['alt']
                    alts.append(alt_allele)
    ...
    strands_made, pam_pos_made = get_made_broke_pams(gens, chrom, ref_genome, CAS_LIST, guide_length, pams_dir)
