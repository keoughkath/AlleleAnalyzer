...
def load_gene_gene_dat(gene_dat_path):
    ...
    """
    gene_gene_dat = pd.read_csv(gene_dat_path, sep='\t', header=0, names=['name', 'chrom', 'txStart', 'txEnd', 'cdsStart', 'cdsEnd', 'exonCount',
...
class Gene:
    ...
    def __init__(self, official_gene_symbol, gene_dat, window):
        ...
        self.chrom = gene_dat.query('index == @self.official_gene_symbol')['chrom'].item()
...
def pair_guides(guides_df, variant1, variant2):
    ...
    'ref_g2','alt_g2','variant_position_in_guide_g2']]
    return guides_out
...
def targ_var(row, cas, hap, level='lax'):
    ...
    if level == 'strict':
        ...
        if hap == 1:
            return annots_file.query('pos == @var')[[f'makes_{cas}',f'var_near_{cas}']].values.any()
        if hap == 2:
            return annots_file.query('pos == @var')[[f'breaks_{cas}',f'var_near_{cas}']].values.any()
...
def main(args):
    ...
    annots_file = pd.read_hdf(annots_file, 'all', where=f'pos >= {MyGene.start} and pos <= {MyGene.end}')
    ...
    vcf_chrom = str(subprocess.Popen(f'bcftools view -H {bcf} | cut -f1 | head -1', shell=True, 
    ...
    for cas in cas_list[1:]:  # skip all because is handled below faster
        ...
        if args['-s']:
            targ_vars_cas = annots_file.query(f'(makes_{cas}) or (breaks_{cas})').pos.tolist()
        else:
            targ_vars_cas = annots_file.query(f'(var_near_{cas}) or (makes_{cas}) or (breaks_{cas})').pos.tolist()
        ...
        for ind in list(inds_w_targ_pair.keys()):
            ...
            if args['--exhaustive']:
                ...
                if args['-s']:
                    ind_cas_targ_pairs['var1_make_pam'] = ind_cas_targ_pairs['var1'].isin(annots_file.query(f'makes_{cas}').pos.tolist())
                    ind_cas_targ_pairs['var2_make_pam'] = ind_cas_targ_pairs[['var2']].isin(annots_file.query(f'makes_{cas}').pos.tolist())
                    ind_cas_targ_pairs['var1_break_pam'] = ind_cas_targ_pairs[['var1']].isin(annots_file.query(f'breaks_{cas}').pos.tolist())
                    ind_cas_targ_pairs['var2_break_pam'] = ind_cas_targ_pairs[['var2']].isin(annots_file.query(f'breaks_{cas}').pos.tolist())
                    ...
                    ind_targ_out.append(ind_cas_targ_pairs.query('both_make and same_hap')[['var1','var2']])
                    ind_targ_out.append(ind_cas_targ_pairs.query('both_break and same_hap')[['var1','var2']])
                    ind_targ_out.append(ind_cas_targ_pairs.query('one_make_one_break_1 and not_same_hap')[['var1','var2']])
                    ind_targ_out.append(ind_cas_targ_pairs.query('one_make_one_break_2 and not_same_hap')[['var1','var2']])
                    ...
                    ind_cas_targ_pairs['both_near_pam'] = ind_cas_targ_pairs[['var1','var2']].isin(annots_file.query(f'var_near_{cas}').pos.tolist()).all(axis=1)
                    ind_targ_out.append(ind_cas_targ_pairs.query('both_near_pam')[['var1','var2']])
                    #   when both make or break, need to be same hap
                    ind_cas_targ_pairs['var1_make_pam'] = ind_cas_targ_pairs['var1'].isin(annots_file.query(f'makes_{cas}').pos.tolist())
                    ind_cas_targ_pairs['var2_make_pam'] = ind_cas_targ_pairs[['var2']].isin(annots_file.query(f'makes_{cas}').pos.tolist())
                    ind_cas_targ_pairs['var1_near_pam'] = ind_cas_targ_pairs['var1'].isin(annots_file.query(f'var_near_{cas}').pos.tolist())
                    ind_cas_targ_pairs['var2_near_pam'] = ind_cas_targ_pairs['var2'].isin(annots_file.query(f'var_near_{cas}').pos.tolist())
                    ind_cas_targ_pairs['var1_break_pam'] = ind_cas_targ_pairs[['var1']].isin(annots_file.query(f'breaks_{cas}').pos.tolist())
                    ind_cas_targ_pairs['var2_break_pam'] = ind_cas_targ_pairs[['var2']].isin(annots_file.query(f'breaks_{cas}').pos.tolist())
                    ind_targ_out.append(ind_cas_targ_pairs.query('(var1_near_pam and var2_make_pam) or (var1_near_pam and var2_break_pam) or (var2_near_pam and var1_make_pam) or (var2_near_pam and var1_break_pam)')[['var1','var2']])
                    ind_cas_targ_pairs['both_make'] = ind_cas_targ_pairs[['var1_make_pam','var2_make_pam']].all(axis=1)
                    ind_cas_targ_pairs['both_break'] = ind_cas_targ_pairs[['var1_break_pam','var2_break_pam']].all(axis=1)
                    ind_cas_targ_pairs['one_make_one_break_1'] = ind_cas_targ_pairs[['var1_make_pam','var2_break_pam']].all(axis=1)
                    ind_cas_targ_pairs['one_make_one_break_2'] = ind_cas_targ_pairs[['var2_make_pam','var1_break_pam']].all(axis=1)
    ...
    if args['-g']:
            if args['-c']:
                logging.info('Cannot generate guides + get targetability simultaneously in cohort-style analysis, only individual-style.')
            else:
    ...
    final_targ.to_hdf(f'{out_prefix}.h5', f'{translated_gene_name}', comp_level=9,complib='blosc')

    # if args['--hap_targs']:
    #     targ_haps.to_hdf(f'{out_prefix}_targ_haps.h5', f'{translated_gene_name}')
