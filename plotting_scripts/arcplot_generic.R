...
p = ggraph(lout) +
  ...
  labs(colour='% population with pair',colors = inferno(256), family='Times',size=12, x='Chromosomal Position', title=out_prefix) +
  scale_x_continuous(breaks=c(gene_start - 5000, gene_start, gene_end, gene_end + 5000), limits=c(gene_start - 5000, gene_end + 5000)) +
  ...
  theme(panel.background = element_rect(fill = NA, colour = 'white'),
        ...
        legend.title=element_text(size=12, family='Times', hjust=0),
        axis.ticks = NULL,
