...
gr = graph_from_data_frame(counts_filt_overall %>% 
  select(var1, var2, n_inds, percent_pop_covered, pop) %>% 
  arrange(pop, percent_pop_covered),
...
p = ggraph(lout, show.legend=FALSE) +
  ...
  theme(panel.background = element_rect(fill = NA, colour = 'white'),
        plot.title = element_text(size=14, face="italic", family='sans'),
        axis.ticks = NULL,
  ...
  facet_edges(~pop, nrow=6, strip.position='left') 

ggsave(paste(out_prefix,'arcplot.pdf',sep=""),plot=p, width=2.8, height=5.5, dpi=300)