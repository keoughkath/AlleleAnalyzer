...
hg38 = Fasta('/pollard/data/vertebrate_genomes/human/hg38/hg38/hg38.fa',
            as_raw=True)

def get_seq(row):
    seq = hg38['chr' + str(chrom)][row['pam_start']:row['pam_stop']]
    return seq

colors = mpl.cm.get_cmap('viridis', len(cas_list)).colors.tolist()

colors_formatted = []

for color in colors:
    colors_formatted.append(str(color[:3]).replace(' ','').replace('[','').replace(']',''))
...
for chrom in chroms:
    for cas in cas_list:
        ...
        chroms_for = []
        chroms_rev = []
        with gzip.open(f'/pollard/data/projects/AlleleAnalyzer_data/pam_sites_hg38/pam_sites_hg38_txt/chr{chrom}_{cas}_pam_sites_for.txt.gz', 'rb') as f:
            for line in f.readlines():
                for_pams.append(int(float(line.strip())) - 1)
                chroms_for.append('chr' + str(chrom))
        with gzip.open(f'/pollard/data/projects/AlleleAnalyzer_data/pam_sites_hg38/pam_sites_hg38_txt/chr{chrom}_{cas}_pam_sites_rev.txt.gz', 'rb') as f:
            for line in f.readlines():
...
bed_df[['chrom','pam_start','pam_stop','name','score','strand','thickStart',
               'thickEnd','itemRgb']].to_csv(f'/pollard/data/projects/AlleleAnalyzer_data/pam_beds_hg38/pams_hg38.bed', sep='\t', header=False, index=False)