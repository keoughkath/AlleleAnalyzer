...
store = pd.HDFStore(f'/pollard/data/projects/AlleleAnalyzer_data/wtc_data/hg19/wtc_formatted_variants_by_chrom/wtc_hg19_formatted_variants_all.h5', complib='blosc', complevel=9)

for chrom in list(range(1,23)) + ['X','Y']:
	df = pd.read_hdf(f'/pollard/data/projects/AlleleAnalyzer_data/wtc_data/hg19/wtc_formatted_variants_by_chrom/chr{chrom}_reformatted.h5')
