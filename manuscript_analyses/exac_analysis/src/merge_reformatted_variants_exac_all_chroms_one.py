...
store = pd.HDFStore(f'/pollard/data/projects/AlleleAnalyzer_data/exac_data/exac_formatted_variants_by_chrom/exac_formatted_variants_all.h5')

for chrom in list(range(1,23)) + ['X','Y']:
	df = pd.read_hdf(f'/pollard/data/projects/AlleleAnalyzer_data/exac_data/exac_formatted_variants_by_chrom/chr{chrom}_reformatted.h5')
